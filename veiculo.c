...
int leituraBinario (FILE* binario, RegistroVeiculo* v, char* removido, int* tam) {
...
void func9(FILE* filevehicle, FILE* filebtree) {
	...
	int prefix;
	while (check(filevehicle)) {

		int pos = ftell(filevehicle);
		RegistroVeiculo* v = create();
		
		if (!leituraBinario(filevehicle, v, &removido, &tam)) continue;
	...
	writeHeader(filebtree, '1', root->RRN, rrn+1);
